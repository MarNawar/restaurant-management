...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		matchStage := bson.D{
			{Key: "$match", Value: bson.D{{}}}, // Empty match stage (or specify your conditions)
		}

		groupStage := bson.D{
			{Key: "$group", Value: bson.D{
				{Key: "_id", Value: bson.D{{Key: "_id", Value: "null"}}},      // Grouping into a single document
		...
		projectStage := bson.D{
			{Key: "$project", Value: bson.D{
				{Key: "_id", Value: 0},         // Exclude _id field
				{Key: "total_count", Value: 1}, // Include total_count in the output
				{Key: "food_items", Value: bson.D{
					{Key: "$slice", Value: []interface{}{"$data", startIndex, recordPerPage}}, // Paginate the array
				}},
			}},
		}
